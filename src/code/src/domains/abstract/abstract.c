    ...
    for (i = 0; i < N_variables; ++i) {
      if (state->component_states[i]) {
      	if (!state->component_states[i]->is_empty_interval)
      	  printf("\t%s=[%ld,%ld]", symbol_table[symbol_table_indices[i]].id, (long) state->component_states[i]->lower,
      		(long) state->component_states[i]->upper);
      	else
          ...
          if (!is_fixed) {
          	if (widening && symbol_table[symbol_table_indices[k]].abstract[i]) {
                  ...
                  if (((AbstractState *) symbol_table[symbol_table_indices[k]].abstract[i])->is_empty_interval)
	          			  ...
	          			  printf("[%ld,%ld] and ", (long) ((AbstractState *) symbol_table[symbol_table_indices[k]].abstract[i])->lower,
                  ...
                  if (previous_state->component_states[k]->is_empty_interval)
                    ...
                    printf("[%ld,%ld]", (long) previous_state->component_states[k]->lower, (long) previous_state->component_states[k]->upper);
                  printf(" = ");
          			}
                  ...
                  if (((AbstractState *) symbol_table[symbol_table_indices[k]].abstract[i])->is_empty_interval)
	          			  ...
	          			  printf("[%ld,%ld]", (long) ((AbstractState *) symbol_table[symbol_table_indices[k]].abstract[i])->lower,
                      (long) ((AbstractState *) symbol_table[symbol_table_indices[k]].abstract[i])->upper);
                  printf("\n");
          			}
    ...
    if (data_flow_matrix[i]) {
      ...
      for (j = 0; j < N_variables; ++j) {
        ...
        if (i == first_abstract_program_point)
          ...
          fprintf(stream, "%s,%s]", ((AbstractState *) symbol_table[symbol_table_indices[j]].abstract[i])->lower == MININT ? "m" :
  ...
  if (data_flow_matrix[0]) {
    ...
    for (j = 0; j < N_variables; ++j) {
      ...
      if (((AbstractState *) symbol_table[symbol_table_indices[j]].abstract[0])->is_empty_interval)
        ...
        fprintf(stream, "%s,%s]", ((AbstractState *) symbol_table[symbol_table_indices[j]].abstract[0])->lower == MININT ? "m" :
