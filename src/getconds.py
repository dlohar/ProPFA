...
def get_all_regions(all_vars,size,each_array,var_no,from_all_folders,order,cur_dir):
    if(var_no==size):        
        ...
        lattes.close()
        input_vol =0
        cmd = './integrate'+ ' --valuation=volume '+ cur_dir+'/latte_input.txt'        
        try:
...
#                reliabs = open(cur_dir+'/singlepaths/'+file_[-1].replace('_f','.txt'))
...
def generate_regions(read_input,from_all_folders,latte,cur_dir):
    f = open(read_input, 'r')
    lines = f.readlines()
    f.close()
    order ={}
...
if __name__ == "__main__":
    ...
    latte='latte-integrale-1.7.3'
    for each in directs:
        if str(each).startswith('latte'):
            latte=str(each)
            break
    latte=cur_dir+'/'+latte+'/dest/bin'
#    latte='/usr2/faculty/soumyajit/debasmita/ProFAPP/latte-integrale-1.7.3/dest/bin'    
       
    read_input='latte_input'
    #read_input = raw_input('Enter the file which has input variables regional distribution: ')  
    all_subregions = generate_regions(read_input,from_all_folders,latte,cur_dir)
    os.chdir(cur_dir)
